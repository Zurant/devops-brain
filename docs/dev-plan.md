...
### 1.1 创建 `src/__init__.py`
空文件，初始化顶层包。

### 1.2 创建 `src/tools/llm_client.py`
...
### 1.4 创建 `src/api/server.py`
FastAPI 基础服务：
- 加载 `.env` (使用 `python-dotenv`)
- 挂载 `src/static/` 目录为静态文件
- 引入路由模块

### 1.5 创建 `src/api/routes/webhook.py`
```
POST /api/webhook
```
...
### 1.6 创建 `src/api/routes/approval.py`
```
POST /api/resume
```
...
### 1.7 创建 `tests/test_graph_basic.py`
编写最基础的图运转测试：
...
- 在内网 GitLab 的目标项目中配置 Webhook，指向 `http://<本机IP>:8000/api/webhook`
- 触发事件选择 `Merge Request events`
- 创建一个测试 MR，验证整个流程端到端跑通

### 4.2 接入 LangFuse
- 在 `src/tools/llm_client.py` 中集成 LangFuse 的回调
- 每次 LLM 调用自动上报 trace（输入 prompt、输出结果、耗时、token 数）
- 在 LangFuse 后台验证能看到完整的调用链路图

### 4.3 完善文档
- 更新 `README.md`：补充完整的架构图、截图、演示步骤
- 编写面试话术要点（可放在 `docs/interview-notes.md`）
