...
审查引擎的后续细化落地计划见 [企业化审查引擎落地计划](enterprise-review-engine-plan.md)，该文档专门描述 Diff Analyzer、Review Planner、分片 Agent 执行与二级汇总。
...
- Phase E7 已开始第一版落地：新增运营概览接口与工作台概览页，可查看任务总量、待审批、高风险、失败任务、GitLab 回写成功/失败与经验库数量。
...
目标：让团队能持续观察审查系统运行质量，而不只是处理单条 MR。

任务：
- 新增 `/api/dashboard`，聚合任务状态、风险分布、项目分布、GitLab 回写结果和经验库规模
- 审批工作台新增“概览”页签
- 展示最近任务与失败回写记录
- 失败回写可从概览直接触发重试

Done When：
- 页面能看到待审批数、高风险数、失败任务数、回写失败数和经验库数量
- 能从概览进入最近任务详情
- 能从概览重试失败 GitLab 评论回写
- `poetry run pytest tests/test_approval_api.py -v` 通过

已完成第一步：
- 新增 `/api/dashboard` 运营概览接口
- 审批工作台新增“概览”页签
- 概览页展示核心指标、状态/风险/项目分布、最近任务和失败回写
- 失败回写记录已携带 `thread_id`，页面可直接发起评论重试
